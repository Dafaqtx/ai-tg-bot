...
// Функция для получения промта для анализа изображений
function getImagePrompt(): string {
  return `Ты - умный ИИ-ассистент в чате Telegram, который анализирует изображения.
...
// Функция для определения типа MIME на основе расширения файла (аудио)
...
// Функция для определения типа MIME изображения
function getImageMimeType(filename: string): string {
  ...
  switch (extension) {
    ...
    case "heif":
      return "image/heif";
    default:
      return "image/jpeg"; // По умолчанию для изображений
  }
}
...
// Функция для обработки изображений с помощью Gemini API
async function processImageMessage(
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), "tg-bot-images");
    if (!fs.existsSync(tempDir)) {
      fs.mkdirSync(tempDir, { recursive: true });
    }
    ...
    // Получаем промт для анализа изображения
    ...
    if (!myfile.uri || !myfile.mimeType) {
      throw new Error("Не удалось загрузить изображение в Gemini API");
    }
    ...
    if (!responseText) {
      return "Извините, не удалось обработать изображение. Попробуйте еще раз.";
    }
...
bot.on("message", async (ctx: BotContext) => {
  try {
    ...
    // Проверяем, является ли сообщение изображением
      ...
      const response = await processImageMessage(fileUrl.href, fileId);
